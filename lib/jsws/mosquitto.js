 ...
 * Also provides protocol support for QoS 1 and 2 but without any guarantees.
 * This means that it will send and receive QoS 1 and 2 messages, but won't
 * retry them if anything fails.
 */
...
Mosquitto.prototype = {
		...
		while(i8V.length > 0 && q < 1000){
			...
			switch(i8V[0] & 0xF0){
				...
				case PUBLISH:
					...
					if(qos == 2){
						this.send_cmd_with_mid(PUBREC, mid);
						// FIXME
					}else{
						if(qos == 1){
							this.send_cmd_with_mid(PUBACK, mid);
						}
						if(this.m.onmessage){
					...
					break;
				case PUBREC:
					var mid = i8V[2]*256 + i8V[3];
					this.send_cmd_with_mid(PUBREL, mid);
					break;
				case PUBREL:
					var mid = i8V[2]*256 + i8V[3];
					this.send_cmd_with_mid(PUBCOMP, mid);
					break;
				case PUBACK:
				case PUBCOMP:
				case SUBACK:
	...
	publish : function(topic, payload, qos, retain){
		...
		var rl = 2+2+topic.length+payload.length;
		if(qos > 0){
			rl += 2;
		}
		var remaining_count = this.get_remaining_count(rl);
